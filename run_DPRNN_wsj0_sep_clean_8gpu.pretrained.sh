...
tag="DPRNNTasNet_wsj0-2mix_sep_clean_new_8gpu_pretrained"  # Controls the directory name associated to the experiment
...
#loadpth=None
loadpth=exp/train_dprnn_DPRNNTasNet_Libri2Mix_enh_single_new_8gpu/_ckpt_epoch_128.ckpt
...
if [[ $stage -le 1 ]]; then
    ...
    CUDA_VISIBLE_DEVICES=$id $python_path train_DPRNN_general.py --exp_dir $expdir \
        --load_path $loadpth \
        --corpus wsj0-mix \
        --model DPRNNTasNet \
        --real_batch_size 24 \
    ...
    echo "wsj0-mix/ConvTasNet" > $expdir/publish_dir/recipe_name.txt
fi
